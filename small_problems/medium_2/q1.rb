...
def longest_sentence(input_text)
  ...
  word_lists = sentences.map do |sentence|
    sentence.split(/\s/)
  end
  sentence_lengths = word_lists.map(&:size)
  max_length = sentence_lengths.max
  longest = sentences[sentence_lengths.index(max_length)]
  puts "#{longest} (#{max_length} words)"
end
