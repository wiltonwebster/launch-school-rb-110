...
# document in a document collection. However, he just started learning to code 
# and somehow his numbers do not match what is expected. He knows you're
...
# The program is incorrectly performing integer division in the IDF method. In
# places where the division is 3/2 and should return 1.5, it is instead 
# returning 1, which causes log to return 0.0. We can fix this by calling to_f
# on either number prior to performing the division.

# We also should be careful about division by 0, if we accidentally enter in a 
# word that doesn't exist in any of the documents. We should implement something
# that checks for and handles that potential exception.